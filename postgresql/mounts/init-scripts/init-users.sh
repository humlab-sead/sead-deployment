...
psql -v ON_ERROR_STOP=1 --username "$PGUSER" <<-EOSQL
    CREATE ROLE humlab_read WITH LOGIN PASSWORD '';
    CREATE ROLE sead_read WITH LOGIN PASSWORD '';
    CREATE ROLE phil WITH LOGIN PASSWORD '';
    CREATE ROLE mattias WITH LOGIN PASSWORD '';
    CREATE ROLE postgrest_anon WITH LOGIN PASSWORD '';
    CREATE ROLE querysead_worker WITH LOGIN PASSWORD '';
    CREATE ROLE querysead_owner WITH LOGIN PASSWORD '';
    CREATE ROLE sead_ro WITH LOGIN PASSWORD '${POSTGREST_DB_PASSWORD}';
    CREATE ROLE clearinghouse_worker WITH LOGIN PASSWORD '';
    CREATE ROLE johan WITH LOGIN PASSWORD '';
    CREATE ROLE postgrest WITH LOGIN PASSWORD '${POSTGREST_DB_PASSWORD}';
    CREATE ROLE anonymous_rest_user WITH LOGIN PASSWORD '';
EOSQL
